...
export async function runDiaryAnalysis(env, notionHeaders, yesterdayStr, sleepInfo) {
    ...
    const fiveDaysBefore = new Date(targetDate);
    fiveDaysBefore.setDate(fiveDaysBefore.getDate() - 5);
    const fiveDaysBeforeStr = fiveDaysBefore.toISOString().split('T')[0];
    ...
    const dbRes = await fetch(
      `https://api.notion.com/v1/databases/${env.NOTION_DB_ID}/query`,
      {
        ...
        body: JSON.stringify({
          filter: {
            and: [
              { property: '날짜', date: { on_or_after: fiveDaysBeforeStr } },
    ...
    const allText = (pastSummary ? `[최근 5일 요약]\n${pastSummary}\n` : '') + targetText;

    const TEXT_CAP = 20000;
    let cappedText = allText;
    if (allText.length > TEXT_CAP) {
      ...
      if (markerIdx > 0 && allText.length - markerIdx < TEXT_CAP) {
        ...
        cappedText = allText.slice(0, TEXT_CAP) + '\n...(이하 생략)...';
      }
    }
...
아래 데이터를 참고해주세요. [분석 대상] 표시된 날짜가 오늘 분석할 일기 전문이고, 그 위 [최근 5일 요약]은 패턴 파악용 요약이에요.

${cappedText}

[분석 대상] 일기를 중심으로 분석해주세요. 최근 5일 요약은 반복 패턴, 이전 제언 중복 방지 용도로만 참고하세요.
...
다음 JSON 형식으로만 응답하세요:
{"에너지레벨":"높음/보통/낮음","사고흐름":"A → B → C (60자 이내)","의미점화단어":"키워드 — 한 줄 (80자 이내)","자기대화톤":"유형 + 한 줄 (80자 이내)","시도결과루프":"(120자 이내)","격려":"(150자 이내)","제언":"▪️ **행동**: (80자 이내)\n▪️ **사고**: (80자 이내)","인지오류":"▪️ (60자 이내)\n▪️ (60자 이내) 또는 빈 문자열","일기요약":"(150자 이내)"}`
