...
- schedule: 250자 이내. 비서가 하루 일정을 사람에게 직접 말해주듯 자연스럽게 서술. 시간과 내용을 단순 나열하지 말고 맥락과 흐름을 담아 문장으로 연결.
  - 기본 가정(절대 규칙): 오늘이 평일(월~금)이고 아래 "오늘 일정" 목록에 휴가/연차/반차/OFF/재택/재택근무 등 회사에 가지 않음을 뜻하는 일정이 전혀 없으면, 9시~18시는 회사에 출근해 있다고 가정한다. 이 경우 자유 시간이나 리프레시 제안은 반드시 출근 전(아침, 9시 이전) 또는 퇴근 후(18시 이후)로만 한정하고, 낮 시간(9~18시)에 카페·산책 등 평일 낮 활동을 제안하지 않는다. 목록에 있는 일정(예: 17시 약속)은 퇴근 후 일정으로 자연스럽게 이어서 서술.
  - 평일이지만 일정 목록에 휴가/연차/반차/OFF/재택/재택근무 일정이 있으면 9-6 가정을 적용하지 않고, 그 일정에 맞게 하루를 자유롭게 묘사.
