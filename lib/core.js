...
export async function extractDiaryData(env, notionHeaders, yesterdayStr, sleepInfo) {
  ...
  const targetDate = new Date(yesterdayStr + 'T00:00:00Z');

  // 이번 주 토요일 계산 (토-금 기준)
  const dow = targetDate.getUTCDay(); // 0=Sun, 6=Sat
  const daysToCurrentSat = dow === 6 ? 0 : dow + 1;
  const thisSaturdayDate = new Date(targetDate);
  thisSaturdayDate.setUTCDate(targetDate.getUTCDate() - daysToCurrentSat);
  const thisSaturdayStr = thisSaturdayDate.toISOString().split('T')[0];

  // 지난 주 토요일 (주간 리포트 조회용)
  const prevSaturdayDate = new Date(thisSaturdayDate);
  prevSaturdayDate.setUTCDate(thisSaturdayDate.getUTCDate() - 7);
  const prevSaturdayStr = prevSaturdayDate.toISOString().split('T')[0];

  // 이번 주 일기(토~어제) + 지난 주 주간 리포트 병렬 조회
  const [dbRes, weeklyRes] = await Promise.all([
    fetch(`https://api.notion.com/v1/databases/${env.NOTION_DB_ID}/query`, {
      method: 'POST',
      headers: notionHeaders,
      body: JSON.stringify({
        filter: {
          and: [
            { property: '날짜', date: { on_or_after: thisSaturdayStr } },
    ...
    fetch(`https://api.notion.com/v1/databases/${WEEKLY_REPORT_DB_ID}/query`, {
      method: 'POST',
      headers: notionHeaders,
      body: JSON.stringify({
        filter: { property: '기간', date: { equals: prevSaturdayStr } }
      })
    })
  ]);

  const [dbData, weeklyData] = await Promise.all([
    dbRes.json(),
    weeklyRes.ok ? weeklyRes.json() : Promise.resolve({ results: [] })
  ]);
  const pages = dbData.results || [];
  const weeklyPages = weeklyData.results || [];
  ...
  async function extractText(blockId, depth, fetchCount, maxFetch) {
    ...
    for (const block of data.results || []) {
      const type = block.type;
      if (type === 'heading_4' && block.heading_4?.is_toggleable &&
          block.heading_4?.rich_text?.some(t => t.plain_text?.includes('모닝브리핑'))) {
        continue;
      }
      if (type === 'column_list') {
  ...
  if (weeklyPages.length > 0) {
    const wr = weeklyPages[0];
    const getText = key => wr.properties[key]?.rich_text?.map(t => t.plain_text || '').join('').trim() || '';
    const wrEnergy = wr.properties['에너지 흐름']?.select?.name || '';
    const wrSummary = getText('주간 요약');
    const wrEnergyPattern = getText('에너지 패턴');
    const wrRecurring = getText('반복 패턴');
    const wrGrowth = getText('성장 포인트');
    const wrAdvice = getText('주간 제언');
    if (wrSummary || wrEnergy) {
      weeklyReportContext += '[지난 주 주간 리포트]\n';
      if (wrEnergy) weeklyReportContext += `에너지 흐름: ${wrEnergy}\n`;
      if (wrSummary) weeklyReportContext += `주간 요약: ${wrSummary}\n`;
      if (wrEnergyPattern) weeklyReportContext += `에너지 패턴: ${wrEnergyPattern}\n`;
      if (wrRecurring) weeklyReportContext += `반복 패턴: ${wrRecurring}\n`;
      if (wrGrowth) weeklyReportContext += `성장 포인트: ${wrGrowth}\n`;
      if (wrAdvice) weeklyReportContext += `지난 주 제언: ${wrAdvice}\n`;
    }
  }

  let allText = '';
  if (weeklyReportContext) allText += weeklyReportContext + '\n';
  if (pastSummary) allText += `[이번 주 흐름]\n${pastSummary}\n`;
  allText += targetText;
  ...
  const TEMPLATE_LINES = new Set([
    '00:00', '내용', '감사한 일', '노력한 일', '칭찬', '나를 위해 한 일', '가장 노력한 일',
    '오늘 할일이 없습니다', '오늘 일정이 없습니다',
  ]);
  const markerIdx = targetText.indexOf('(분석 대상)');
  const afterNl = markerIdx >= 0 ? targetText.indexOf('\n', markerIdx) : -1;
  const afterMarker = afterNl >= 0 ? targetText.slice(afterNl + 1).trim() : '';
  const hasContent = afterMarker.split('\n').some(line => {
    const t = line.trim();
    return t && !/^\[.+\]$/.test(t) && !TEMPLATE_LINES.has(t);
  });
...
- "다음 진료 때 이 내용을 보여주세요/공유해보세요" 같은 제안은 어떤 항목에서도 하지 마세요.
...
일기 구조 안내:
- 일기는 세 섹션으로 나뉘어요: [☀️ 모닝브리핑], [📝 기록], [🌙 일기]
- [☀️ 모닝브리핑]은 하루 시작 전 할일과 일정이에요.
- [📝 기록]은 하루 중 틈틈이 기록한 짧은 메모들이에요. 감정이나 상황, 생각이 담겨있어요.
- [🌙 일기]는 하루를 마무리하며 쓴 감사한 일과 노력한 일이에요.
- [🌙 일기] 섹션에 내용이 있으면 그것을 독립적인 긍정 신호로 읽어주세요.
- 각 섹션의 분위기 차이도 심리 상태의 단서로 봐주세요.

시도&결과 루프 작성 규칙:
- [☀️ 모닝브리핑]에 할일 목록이 있으면, [📝 기록]이나 [🌙 일기]에서 실제로 어떻게 됐는지 단서를 찾아 비교해주세요.
- "계획했던 일들이 실제 기록에 어떻게 이어졌는지" 따뜻한 관찰 톤으로 써주세요.
- 모닝브리핑이 없거나 비어 있는 날은 이 항목을 빈 문자열로 두세요.
...
{"에너지레벨":"높음/보통/낮음","사고흐름":"A → B → C 형식으로","의미점화단어":"키워드 — 왜 그렇게 읽혔는지 한 줄","자기대화톤":"격려형/명령형/비판형 등 + 한 줄","시도결과루프":"Plan vs Goal 비교 + 완료율 + 시간 빠름/늦음","격려":"","제언":"▪️ **행동**: (근거 포함 한 줄)\n▪️ **사고**: (근거 포함 한 줄)","인지오류":"▪️ 첫 번째\n▪️ 두 번째 또는 빈 문자열","일기요약":""}`
    : `당신은 민영님의 심리상담사입니다. 어제 일기에 기록된 내용이 없어요. 최근 기록 흐름을 참고해서 따뜻한 격려 한 마디와 내일을 위한 제언을 작성해주세요.
- "다음 진료 때 이 내용을 보여주세요/공유해보세요" 같은 제안은 하지 마세요.

${allText}

제언 형식: "▪️ **행동**: (근거 포함 한 줄)\n▪️ **사고**: (근거 포함 한 줄)"

JSON만 응답 (격려·제언 외 나머지는 빈 문자열): {"에너지레벨":"","사고흐름":"","의미점화단어":"","자기대화톤":"","시도결과루프":"","격려":"","제언":"▪️ **행동**: ...\\n▪️ **사고**: ...","인지오류":"","일기요약":""}`;
  ...
  const props = hasContent ? {
    ...
    '격려': { rich_text: parseRichText(analysis['격려'] || '') },
    '제언': { rich_text: parseRichText(analysis['제언'] || '') },
    '일기 요약': { rich_text: parseRichText('오늘 기록이 없는 날이에요') },
    '분석 완료': { checkbox: true }
  };

  if (hasContent && analysis['에너지레벨'] && ['높음', '보통', '낮음'].includes(analysis['에너지레벨'])) {
...
function getWeekRange(isoDate) {
  ...
  // 가장 최근 완료된 금요일까지의 일수 (오늘이 금요일이면 0, 토요일이면 1)
  const daysToLastFri = (dow + 2) % 7;
  const friday = new Date(d);
  friday.setUTCDate(d.getUTCDate() - daysToLastFri);
...
function formatWeekLabel(saturday, friday) {
  const year = saturday.getUTCFullYear();
  const month = saturday.getUTCMonth() + 1;
  const firstOfMonth = new Date(Date.UTC(year, saturday.getUTCMonth(), 1));
  const firstDow = firstOfMonth.getUTCDay() === 6 ? 1 : (firstOfMonth.getUTCDay() + 2); // 토 기준 주차
  const weekOfMonth = Math.ceil((saturday.getUTCDate() + firstDow - 2) / 7);
  const pad = n => String(n).padStart(2, '0');
  const startLabel = `${pad(saturday.getUTCMonth() + 1)}.${pad(saturday.getUTCDate())}`;
  const endLabel = `${pad(friday.getUTCMonth() + 1)}.${pad(friday.getUTCDate())}`;
...
function buildWeeklyPageBlocks(entries) {
...
export async function handleWeeklyReport(request, env, notionHeaders) {
  ...
  // iso_date 정제: 공백/숨은문자 제거 후 YYYY-MM-DD 형식 추출
  const dateMatch = String(iso_date).trim().match(/(\d{4})-(\d{1,2})-(\d{1,2})/);
  const month = dateMatch ? parseInt(dateMatch[2], 10) : 0;
  if (!dateMatch || month < 1 || month > 12) {
    return new Response(JSON.stringify({
      error: 'iso_date 형식 오류 (yyyy-MM-dd 필요, mm은 분·MM이 월)',
      received: String(iso_date)
    }), { status: 400, headers: { 'Content-Type': 'application/json' } });
  }
  const cleanDate = `${dateMatch[1]}-${String(month).padStart(2,'0')}-${String(parseInt(dateMatch[3],10)).padStart(2,'0')}`;

  const { saturday, friday, saturdayStr, fridayStr } = getWeekRange(cleanDate);
  ...
  const existingRes = await fetch(`https://api.notion.com/v1/databases/${WEEKLY_REPORT_DB_ID}/query`, {
    ...
    headers: notionHeaders,
    body: JSON.stringify({
      filter: { property: '기간', date: { equals: saturdayStr } }
    })
  });
  ...
  const dbRes = await fetch(`https://api.notion.com/v1/databases/${env.NOTION_DB_ID}/query`, {
    ...
    body: JSON.stringify({
      filter: {
        and: [
          { property: '날짜', date: { on_or_after: saturdayStr } },
          { property: '날짜', date: { on_or_before: fridayStr } }
        ]
      },
      sorts: [{ property: '날짜', direction: 'ascending' }]
    })
  ...
  const prompt = `당신은 민영님의 심리상담사입니다. 이번 주(${saturdayStr} ~ ${fridayStr}) 일기 분석 데이터를 바탕으로 주간 리포트를 작성해주세요.
...
{"주간요약":"이번 주를 한두 문장으로","에너지흐름":"높음/보통/낮음/혼합 중 하나","에너지패턴":"에너지 변화 흐름 한두 줄","반복패턴":"반복된 감정·행동·사고 패턴 (없으면 빈 문자열)","성장포인트":"이번 주 잘한 점 (없으면 빈 문자열)","주간격려":"따뜻한 격려 한두 마디","주간제언":"▪️ **행동**: ...\\n▪️ **사고**: ..."}`;
  ...
  let claudeData = await claudeRes.json().catch(() => ({}));
  ...
  if (!claudeRes.ok && [429, 529].includes(claudeRes.status)) {
    ...
    claudeData = await claudeRes.json().catch(() => ({}));
  }
  ...
  // max_tokens 초과로 잘린 경우 닫는 } 보완 후 파싱 시도
  const isTruncated = claudeData.stop_reason === 'max_tokens';
  let textToParse = rawText;
  if (isTruncated && rawText.includes('{') && !rawText.trimEnd().endsWith('}')) {
    textToParse = rawText.trimEnd().replace(/,?\s*"[^"]*"?\s*$/, '') + '}';
  }

  const jsonMatch = textToParse.match(/\{[\s\S]*\}/);
  if (!jsonMatch) {
    return new Response(
      JSON.stringify({ error: 'Claude 응답에 JSON 없음', raw: rawText, stop_reason: claudeData.stop_reason }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: 'JSON 파싱 실패', raw: jsonMatch[0], stop_reason: claudeData.stop_reason }), { status: 500 });
  }
  ...
  const notionRes = await fetch('https://api.notion.com/v1/pages', {
    ...
    body: JSON.stringify({
      ...
      children: buildWeeklyPageBlocks(analyzed)
    })
  });
