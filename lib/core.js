...
export async function handleWeeklyReport(request, env, notionHeaders) {
  ...
  if (!dateMatch) {
    return new Response(JSON.stringify({ error: 'iso_date 형식 오류', received: String(iso_date) }), {
      status: 400, headers: { 'Content-Type': 'application/json' }
    });
  }
  const cleanDate = `${dateMatch[1]}-${String(dateMatch[2]).padStart(2,'0')}-${String(dateMatch[3]).padStart(2,'0')}`;
