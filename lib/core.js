...
export async function handleWeeklyReport(request, env, notionHeaders) {
  ...
  const prompt = `당신은 민영의 심리상담사입니다. 이번 주(${mondayStr} ~ ${sundayStr}) 일기 분석 데이터를 바탕으로 주간 리포트를 작성해주세요.

참고 사실: 민영은 불안과 우울로 정신과 치료를 받고 있어요. 분석할 때 이 맥락을 염두에 두되, 병리적 해석보다는 일상의 흐름 안에서 자연스럽게 읽어주세요.
