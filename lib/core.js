...
export async function extractDiaryData(env, notionHeaders, yesterdayStr, sleepInfo) {
  ...
  const thisMondayDate = new Date(targetDate);
  thisMondayDate.setUTCDate(targetDate.getUTCDate() - (dow - 1));
  const thisMondayStr = thisMondayDate.toISOString().split('T')[0];

  // 지난 주 월요일 (주간 리포트 조회용)
  const prevMondayDate = new Date(thisMondayDate);
  prevMondayDate.setUTCDate(thisMondayDate.getUTCDate() - 7);
  const prevMondayStr = prevMondayDate.toISOString().split('T')[0];

  // 이번 주 일기 + 지난 주 주간 리포트 병렬 조회
  const [dbRes, weeklyRes] = await Promise.all([
    fetch(`https://api.notion.com/v1/databases/${env.NOTION_DB_ID}/query`, {
      ...
      body: JSON.stringify({
        filter: {
          and: [
            { property: '날짜', date: { on_or_after: thisMondayStr } },
    ...
    fetch(`https://api.notion.com/v1/databases/${WEEKLY_REPORT_DB_ID}/query`, {
      ...
      headers: notionHeaders,
      body: JSON.stringify({
        filter: { property: '기간', date: { equals: prevMondayStr } }
      })
    })
  ]);
